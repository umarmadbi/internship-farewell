...
/* Finds a person in PEOPLE regardless of how the key's letters are
   capitalized or spaced (e.g. "Alastair", "alastair" and "ALASTAIR"
   all match the same entry). Returns null if nothing matches. */
function findPerson(chosenName){
  if (typeof PEOPLE === 'undefined' || !chosenName) return null;
  const target = chosenName.trim().toLowerCase();
  const foundKey = Object.keys(PEOPLE).find(k => k.trim().toLowerCase() === target);
  return foundKey ? PEOPLE[foundKey] : null;
}

function startExperience(chosenName){
  const person = findPerson(chosenName);
  // Prefer an explicit displayName (useful when the URL uses an opaque
  // link token instead of the person's real name — see messages.js)
  name = (person && person.displayName) ? person.displayName : (chosenName || "there");
  MESSAGES = person ? person.messages : (typeof DEFAULT_MESSAGES !== 'undefined' ? DEFAULT_MESSAGES : []);
...
/* Caps the bubble's size to whatever space is actually left to the
   right of and below the character, so it can never run off the
   edge of the screen — long messages scroll internally instead. */
function fitBubble(){
  const charRect = character.getBoundingClientRect();
  const marginV = 20;
  const gapToCharacter = 22; // must match .bubble's `left: calc(100% + 22px)` in CSS
  const edgeBuffer = 12;     // extra breathing room from the screen edge

  const availableH = window.innerHeight - charRect.top - marginV;
  const maxH = Math.max(80, Math.min(availableH, window.innerHeight * 0.7));
  bubble.style.maxHeight = maxH + 'px';

  const availableW = window.innerWidth - charRect.right - gapToCharacter - edgeBuffer;
  const width = Math.max(90, Math.min(availableW, 260));
...
function buildLetter(){
  letterEyebrow.textContent = "One Last Note from Umar";
